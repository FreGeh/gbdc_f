...
private:
    ...
    // Oriented state hash for stabilization (keeps (p,n) order)
    ...
    // Canonical state hash for final signature (flip invariant)
...
public:
    IsoHash2(const CNFFormula& formula, const IsoHash2Settings& s) :
        settings(s),
        cnf(formula),
        color_functions{ColorFunction(cnf.nVars() + 1), ColorFunction(cnf.nVars() + 1)},
    ...
    Stats run() {
        ...
        while (stats.round < settings.max_iterations) {
            ...
            if (settings.print_stats) {
                std::cerr << "c Round " << stats.round << " partitions: " << prev_partition_count << "\n";
            }
